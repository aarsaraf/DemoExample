CLASS lhc_BusinessObjectHeader DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR BusinessObjectHeader RESULT result.
...
CLASS lsc_ZR_BIZ_OBJECT_HDR DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZR_BIZ_OBJECT_HDR IMPLEMENTATION.
  ...
  METHOD save_modified.
    ...
    IF update-businessobjectheader IS NOT INITIAL.
      ...
      SELECT SINGLE * FROM zr_biz_object_hdr WHERE BusinessObjectID = @lv_objid  INTO @DATA(ls_header).
      IF sy-subrc = 0.
    ...
    IF update-businessobjectitem IS NOT INITIAL.
      ...
      SELECT SINGLE * FROM zr_biz_object_item WHERE BusinessObjectID = @lv_hdr AND
